...
from sklearn import svm #Question 4a
...
for c in range(1,202,5):
    for g in range(21):
        print("\nC=%d and gamma=%.2f"%(c,g/2))#debug
        ...
        for i, (train_ind, test_ind) in enumerate(kf.split(features)):
            ...
            svmclf = svm.SVC(kernel="rbf", C=c, gamma=g/2)
...
print("\nBest Geometric Mean for C = %d and Gamma = %.2f" %(int(max_geo_idx / 20)*5 +1 , (max_geo_idx % 20)/2))
...
print("\nBest Sensitivity for C = %d and Gamma = %.2f" %(int(max_sens_idx / 20)*5 +1 , (max_sens_idx % 20)/2))
...
print("\nBest Specificity for C = %d and Gamma = %.2f" %(int(max_spec_idx / 20)*5 +1 , (max_spec_idx % 20)/2))
